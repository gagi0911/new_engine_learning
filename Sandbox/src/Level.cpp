...
void Level::Init()
{
	...
	m_Player.LoadAssets();
	int PreLoad = 5;
	m_Pillars.resize(PreLoad);
	for (int i = 0; i < PreLoad; i++)
		CreatePillar(i, i * 10.0f);
	audio = &Hazel::AudioPlayer::GetInstance();
	audio->LoadAudioFile("assets/audio/detective.mp3");
	audio->Play();
}
...
void Level::OnRender()
{
	...
	{
		Hazel::Renderer2D::DrawRotatedQuad(pillar.TopPosition, pillar.TopScale, glm::radians(180.0f), m_TriangleTexture, 1.0f, color);
		Hazel::Renderer2D::DrawRotatedQuad(pillar.BottomPosition, pillar.BottomScale, 0.0f, m_TriangleTexture, 1.0f, color);
	}
...
void Level::CreatePillar(int index, float offset)
{
	...
	// divide by z axis or it will cover incorrectly
